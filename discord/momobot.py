...
async def on_message(message):
    ...
    if args.startswith('!setchannel'):
        ...
        await message.channel.send( 'Meow, now lurking');
        ...
        await message.channel.send( 'https://giant.gfycat.com/EqualDemandingAiredale.webm')
    elif args.startswith('!meow'):
        await message.channel.send( 'Mrrrowl')
    elif args.startswith('!hoppes') or args.startswith('!thankmew'):
        await message.channel.send( 'https://yt3.ggpht.com/-P7lfEaDGB6M/AAAAAAAAAAI/AAAAAAAAAAA/qZZA1cFajM4/s900-c-k-no-mo-rj-c0xffffff/photo.jpg')
    elif args.startswith('!brickle') or args.startswith('!iulius'):
        rand = random.randint(0,3)
        if rand > 2:
          await message.channel.send( 'https://www.youtube.com/watch?v=992xtvLAfKo');
        if rand > 1:
          await message.channel.send( 'https://www.youtube.com/watch?v=mxzgwJ8tSE0');
        elif rand > 0:
          await message.channel.send( 'https://www.youtube.com/watch?v=yO7MWuJ7zLA');
        else:
          await message.channel.send( 'https://www.youtube.com/watch?v=IIKPA8Z-D0g');
        #if rand > 0:
        #    await message.channel.send( 'https://giphy.com/gifs/gifnews-dinosaurs-nEUMeGI3r3YAw')
        #else:
        #    await message.channel.send( 'http://media.giphy.com/media/Zg7clvqHE3CdW/giphy.gif')
    elif args.startswith('!haywood'):
        await message.channel.send( 'https://giphy.com/gifs/coding-srbiWWa0VW2YM')
        ...
        tot = i + f
        await message.channel.send( 'This %s is a good girl, rated %s/10: %s' % (label, i, url))
        ...
        f = math.floor(math.fabs(random.gauss(0,1)))
        tot = i + f
        await message.channel.send( 'This %s is a good boy, rated %s/10: %s' % (label, i, url))
    elif args.startswith('!goodtora') or args.startswith('!goodtiger'):
        url = random.choice(toraCache)
        i = random.randint(10,12)
        f = math.floor(math.fabs(random.gauss(0,1)))
        tot = i + f
        await message.channel.send( 'This Daitora is a good tiger, rated %s/10: %s' % (tot, url))
    elif args.startswith('!goodbilly'):
        url = random.choice(toraCache)
        i = random.randint(10,12)
        f = math.floor(math.fabs(random.gauss(0,1)))
        tot = i + f
        await message.channel.send( 'This Billy is a good tiger, rated %s/10: %s' % (tot, url))
        ...
        await message.channel.send( 'Bad to the bone %s' % url)
        ...
        await message.channel.send( '$mmomo')
    elif args.startswith('!isatest'):
        await other['home'].send_message('Test complete! MROWL!');
    elif args.startswith('!now'):
        await message.channel.send( '%s' % int(time.time()));
    else:
        #Check if "khadgar" is somewhere in the text
        text = message.content.lower().split(' ')
        if "khadgar" in text:
            await message.channel.send( 'I think you mean Dadgar')
